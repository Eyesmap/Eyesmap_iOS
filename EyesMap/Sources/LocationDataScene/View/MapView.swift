...
class MapView: UIImageView {
    ...
    @objc func selectFunc(_ sender: UIButton) {
        ...
        if(sender.backgroundColor == UIColor(red: 250/255, green: 207/255, blue: 6/255, alpha: 1)) {
            ...
            self.delegate?.jachiViewAppear(isHidden: true, title: "", gu_Id: 0)
        } else {
            sender.backgroundColor = UIColor(red: 250/255, green: 207/255, blue: 6/255, alpha: 1)
            sender.setTitleColor(UIColor.black, for: .normal)
            
            // 지도 위 버튼을 눌렀을 때 자치뷰 띄우기(title, 구별 Id)
            if let index = MapView.jachiArray.firstIndex(of: sender) {
                self.delegate?.jachiViewAppear(isHidden: false,
                                               title: (sender.titleLabel?.text)?.components(separatedBy: " ")[0] ?? "",
                                               gu_Id: index)
            }
        }
    }
